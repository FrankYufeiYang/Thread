...
// reference: tutorialspoint website on floating point comparion 
bool compare_FP(double x, double y, float epsilon = 0.00000001){
	if(fabs(x - y) < epsilon){
...
{	
	...
	printf("Mean: %f, %f, Check %s \n", res_thread->mean, res_serial->mean, 
			(compare_FP(res_thread->mean,res_serial->mean)?"passed":"failed") );
			
	printf("Standard Deviation: %0.9f, %0.9f, Check %s\n", res_thread->stddev, res_serial->stddev, 
			(compare_FP(res_thread->stddev,res_serial->stddev)?"passed":"failed"));
